...
export const zhCn: Bundle = {
  ...
  seed: {
    ...
    reimportBanner:
      "本机已无法解锁存储的种子 — 系统密钥库中的密钥丢失（数据文件夹被移动到了新机器，或钥匙串被重置）。请重新输入你的助记词，在本机上重新设置。你的资金和交易不受影响。",
    wordCount: "{n} 个词",
    wordCountHint:
  ...
  settings: {
    ...
    feeMax: "最高费率（sat/vB）",
    feeMaxHint:
      "注资费用加价的上限（HTLC + adaptor）；默认 500，同时也是系统硬上限。调低它可控制注资成本。赎回和退款加价会忽略它——临近时间锁时，它们会支付所需的任何费用，最高可达所涉价值，因此绝不会因费用上限而损失交换的某一方。",
    feeReservation: "注资加价预留（×）",
    feeReservationHint:
      "仅限注资加价（HTLC + adaptor）。既是资金检查作为加价余量预留的余额，也是一次注资加价的上限（其原始费率的该倍数）。越高越能挽救更大的费用飙升，但会占用更多余额并拒绝更多交换。默认 3。",
    feeSave: "保存",
    feeSaving: "正在保存…",
    feeSaved: "已保存",
    feeReset: "重置为默认值",
    ...
    protocols: "交换协议",
    protocolsHint:
      "此构建所支持的协议 — 数字是各协议的消息格式版本（并非协议的等级排序）。版本不同的报价可以浏览但不能接受；交换双方必须一致。",
    machine: "本机",
    machineHint:
  ...
  wallets: {
    ...
    degradedViewsTip: "{total} 台中有 {down} 台 Electrum 服务器离线 — 备用服务器正在接管。",
    intro:
    ...
    sendOverBalance: "超过可用余额（含预估手续费）。",
    feeLabel: "网络手续费 — 在金额之外另行支付",
    fee_slow: "慢速",
    fee_normal: "普通",
    fee_fast: "快速",
    fee_custom: "自定义",
    feeRate: "{rate} sat/vB",
    feeNoEstimate: "暂无数据",
    feeNoEstimatesNote: "当前没有实时费率估算 — 手续费市场可能为空。请设置自定义费率。",
    feeCustomLabel: "自定义费率（sat/vB）",
    feeCustomMin: "最低 {min} sat/vB。",
    sendFeePreview: "预估网络手续费：一笔普通交易约 {fee} {sym}。",
    sendReview: "预览",
    sendBack: "返回",
    sendConfirmTitle: "确认发送（{sym}）",
    sendConfirmRecipient: "收款方",
    sendConfirmAmount: "金额",
    sendConfirmFee: "网络手续费（预估）",
    sendConfirmFeeValue: "约 {fee} {sym}（{rate} sat/vB）",
    sendConfirmTotal: "合计（约）",
    sendIrreversible: "交易不可撤销。请仔细核对地址 — 发错地址的币将无法找回。",
    ...
    bump: "加速",
    bumpHint: "支付更高的手续费，让这笔待确认交易更快确认（RBF）。",
    bumpTitle: "提高手续费（{sym}）",
    bumpIntro: "用一笔支付更高手续费的交易替换这笔待确认交易（RBF）。当前费率约 {rate} sat/vB。",
    bumpNeedHigher: "请选择高于当前约 {rate} sat/vB 的费率。",
    bumpBroadcast: "手续费已提高 — 替换交易 {txid}… 正在路上（{sym}）。",
    bumpConfirm: "提高手续费",
  },
  corkboard: {
  ...
  relays: {
    title: "网络",
    tabNostr: "Nostr",
    serversSubtitle: "{coin} 的 Electrum 服务器 — 钱包主服务器、活跃视图和冷备份。健康状态仅从真实流量中观测；备用服务器在被提升前绝不会被连接。",
    healthyCount: "{healthy} / {total} 健康",
    roleWallet: "钱包",
    roleView: "视图",
    roleStandby: "备用",
    stateHealthy: "健康",
    ...
    stateUntested: "未测试",
    retryIn: "{secs} 秒后重试",
    serverTip: "{requests} 次请求 · {failures} 次失败",
    serverErr: "最近错误：{error}",
    noServers: "此币种未配置 Electrum 服务器。",
