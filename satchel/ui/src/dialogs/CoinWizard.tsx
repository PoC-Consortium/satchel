...
// A swap needs two chains, so two live coins is the bar for trading — the
// progress bar tracks toward it. It is NOT a gate here (#119): the wizard is a
// first-run nudge, and Continue/Later always proceeds.
const MIN_COINS = 2;

// First-run coin-setup dialog: shown once (onboarded flag), after the merchant
// + seed steps. Lists every coin the engine supports (from listcoins) joined
// with the connection templates (coins.toml); picking one opens the structured
// CoinSetup pre-filled from its template. A template whose coin the engine
// doesn't know is shown as unsupported. Unlike the old hard coin wall, this
// never blocks: you can set up two coins now or click "Later" and do it in
// Settings › Coins — trading is gated per-action, not by this dialog.
  ...
  return (
    <Dialog open maxWidth="sm" fullWidth>
      ...
      <Box sx={{ display: "flex", alignItems: "center", px: 3, pb: 2, pt: 1, gap: 1 }}>
        <Box sx={{ flex: 1 }} />
        {/* Continue is ALWAYS enabled — coin setup is a first-run nudge, not a
            wall (#119). With fewer than two live coins the primary reads "Later"
            (proceed now, set up in Settings › Coins); once two are live it reads
            as the confident "you're ready to trade" Continue. */}
        <Button variant="contained" onClick={() => void onDone()}>
          {liveCount >= MIN_COINS ? t("coinWizard.continue") : t("common.later")}
        </Button>
      </Box>
