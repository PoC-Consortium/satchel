...
pub enum AdaptorState {
    ...
    /// (`adaptor_assemble` jumps Accepted → Signed directly). v2 executes
    /// ENTIRELY within this state — funding and settlement are sub-divided
    /// by the progress display, never by state transitions.
    Signed,
