#!/usr/bin/env python3
"""Upgrade-path e2e: swaps STARTED on the last released pactd (v0.1.0-rc19,
built from the immutable tag into bin/pactd-rc19) must finish on the CURRENT
build — the compat promise a stable release makes. Three cells:

  UpgradeMidSwapV1     both parties on rc19 drive a v1 swap to both-funded,
                       both daemons stop, the SAME datadirs relaunch on the
                       current build and the swap completes on the ORIGINAL
                       fundings (rc19-written DB: swaps, outbox, meta,
                       machine.json all reopened + migrated).
  UpgradeMidSwapV2     the v2 twin, upgraded at committed — the state-richest
                       DB an rc19 could leave behind (nonce sessions +
                       assembled adaptor sigs), held observable by the
                       maker's btc=3 conf gate.
  MixedVersionSwapV1   an rc19 maker completes a whole v1 swap against a
...
from framework.binaries import find_pactd_rc19  # noqa: E402
    ...
    try:
        ...
        for _ in range(60):
            ...
            if handshake_done(maker, taker):
                mine(h)
        assert sid, "rc19 pair never reached both-legs-funded"
        pre_a = swap_of(maker, sid)["htlc_a_txid"]
        pre_b = swap_of(taker, sid)["htlc_b_txid"]

        # --- the upgrade: stop BOTH rc19 daemons, relaunch the same datadirs
        # (same ports, same DBs) on the current build. ---
        print(f"[upgrade-e2e] both funded on rc19 ({sid[:16]}) — upgrading")
        maker.stop()
        taker.stop()
        ...
        print("[upgrade-e2e] v1 upgrade OK: rc19-started swap completed on the "
...
def scenario_upgrade_mid_swap_v2(h):
    """v2 upgraded at committed — rc19 wrote the nonce sessions and assembled
    ...
    try:
        ...
        for _ in range(60):
            ...
            if handshake_done(maker, taker) and not leg_b_on_wire():
                mine(h)
        assert sid, "rc19 pair never reached v2 committed"
        pre_a = swap_of(maker, sid)["funding_a_txid"]
        pre_b = swap_of(taker, sid)["funding_b_txid"]

        print(f"[upgrade-e2e] v2 committed on rc19 ({sid[:16]}) — upgrading")
        maker.stop()
        taker.stop()
        ...
        print("[upgrade-e2e] v2 upgrade OK: completed via the rc19-written "
...
def scenario_mixed_version_swap_v1(h):
    """Wire compat: an rc19 maker and a CURRENT-build taker complete a whole
    ...
    try:
        ...
        for _ in range(60):
            ...
            raise AssertionError(
                ...
                f"rc19-maker={swap_of(maker)} current-taker={swap_of(taker)}")
        print("[upgrade-e2e] mixed-version OK: rc19 maker <-> current taker "
