...
Yes, safely — it's built for **failover and recovery**, not trading from two machines at once. Each machine independently drives only the swaps it started; the others show those swaps read-only in the swaps dock under an **"Another machine"** group, tagged with that machine's short id (shown in **Settings → About → This machine**). A machine never touches another's in-flight swaps, and two machines can never reuse the same swap secrets — each install gets its own private key-derivation scope, so a swap on one machine can't be double-funded or stolen from another. If a machine dies, open its group and click **Take over** — after confirming it's really stopped — and this machine drives its swaps to completion or refund. Take-over never skips a swap: a Private/v2 swap whose payout wallet this machine doesn't control — or can't positively prove it controls — is adopted anyway, with a refund-only notice in the activity log, but **refund-only** — completing it would pay the other machine, so this machine rides it to its timelock refund, which pays an address it owns (all v1 swaps and seed-derived v2 swaps adopt fully). The check repeats on every scheduler pass, so attaching the owning wallet later re-enables normal completion without another take-over. Swaps with nothing locked yet are also safe to take over: the confirm dialog says so honestly — they continue only if this wallet can positively verify their funding is safe to send, and otherwise end in a clean abort about 15 minutes after take-over; no funds are at risk either way. The hand-off also works in reverse: a machine restarted after another same-seed machine settled its swaps reconciles each one against the chain before acting — once per start, with zero re-broadcasts — so a returning owner sees those swaps close out at their true final state instead of lingering or attempting a doomed refund. This holds even for a coin backed only by your own node (no address index): when the engine knows which output funded a leg, it scans full blocks to prove the leg was already spent — so a swap another machine settled never imports as a ghost row, a stuck taken-over swap heals itself on the next restart, and an "Another machine" row is cleared only once its settlement is confirmed at depth. Recovery works from the seed alone: reinstall, import your recovery phrase, and your in-flight swaps come back from the encrypted relay snapshots — a fresh install is a new machine, so they appear under "Another machine" (the old install's) and one **Take over** puts this machine in charge of them. Because both machines share one seed they share **one wallet balance** — running two active traders doesn't give you double the liquidity, and a standby machine may show a slightly stale balance until it rescans. Withdrawals and receives work on every machine at all times. Upgrading from a pre-multi-machine release? Your finished swaps stay in the Swaps ledger automatically on first start; only a swap that was still active at upgrade time appears under "Another machine" and needs one **Take over**.

**Satchel says it can't unlock my seed anymore — what now?**
The no-passphrase seed is wrapped under a key in the OS keystore (Windows/macOS), and that key doesn't travel: if the data folder was moved to a new machine or the keychain was reset, the stored seed can't be decrypted. Satchel detects this at startup and opens a guided re-import — re-enter your recovery phrase and it's set up again on this machine. Funds and swaps are not affected; any of this machine's own in-flight swaps reappear under **"Another machine"** (recovered after re-import) and need one **Take over**. See [Security Model](Security-Model).
