...
- **Wallets** — per-coin balances (one card per configured coin) with **Send** and **Receive** on every card. Node-backed coins show which node wallet their RPCs are scoped to; Electrum-connected coins show **pact seed wallet** (the wallet lives on your recovery phrase — no node) and add an **Activity** transaction history.
