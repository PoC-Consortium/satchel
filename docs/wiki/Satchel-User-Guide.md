...
Nothing is committed until the final confirm: the merchant and its seed are created together at the wizard's last step, so cancelling anywhere earlier — even after the phrase was shown — leaves no trace, and the previously active merchant keeps its slot.
...
Contacts are **local only**. They live on this device and are never shared, published, signed, or sent to a relay. The nickname is shown **alongside** the identicon and fingerprint and never replaces them: the identicon + fingerprint remain the real identity, which is what defends against impersonation (a stranger can copy a nickname but not the key). The standing is **soft and personal** — the protocol itself derives trust only from atomicity, so the chain enforces the deal regardless of how you've labelled someone. The one engine-side effect: your **Blocked** set is mirrored into the engine, which enforces it on your own offers (below).

> **Blocking works one way.** A counterparty you've blocked **cannot take your offers**: their take is ignored silently — they get no reply at all (to them you simply look unresponsive), your offer stays live for everyone else, and a `take-blocked` entry in your daemon log is the only trace. In the other direction it's only a reminder: their offers are hidden when **Hide blocked offers** is on, and taking one shows a warning plus an extra confirm — but nothing stops you, and the swap is atomic either way.
