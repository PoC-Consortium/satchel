...
- **v2 cooperative redeem is not RBF-bumpable.** Its fee is sealed into the pre-signed adaptor signature. The fee commits at the live market rate at swap start, and a CPFP child bumps the redeem if the network gets busy. (The v2 single-key refund *is* bumpable; v1 redeem and refund are both bumpable.)
