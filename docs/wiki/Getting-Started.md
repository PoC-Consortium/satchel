...
2. **Create a merchant + seed.** On first run, Satchel walks you through creating a *merchant* (one trading identity = one seed = one data dir). Write down the recovery phrase, then choose **No passphrase** (simplest) or **Encrypt** (a passphrase you'll type each session). The merchant only comes into existence when you confirm the final step — cancelling the wizard earlier leaves nothing behind.
3. **Connect at least two coins.** Browsing the board works out of the box, but **trading needs two live coins** (e.g. BTCX and BTC). For each, choose **your own node** (point Satchel at its RPC) or **Electrum servers** (no node — the wallet lives on your Pact seed) and let it validate the connection. See [Configuring Coins](Configuring-Coins).
   - *Just looking?* Skip coin setup — you land straight in the app and can read the whole board with zero coins configured. Posting, taking, and funding stay gated until two coins are live.
