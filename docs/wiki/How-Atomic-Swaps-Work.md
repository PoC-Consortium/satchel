...
Pact ships **two protocol versions**, both reviewed and running on mainnet. For a given pair the engine prefers HTLC and only uses the adaptor route for Taproot-only pairs.
...
| Cooperative redeem RBF-bumpable | Yes | **No** (sealed into the pre-signed sig and broadcast non-replaceable; the fee commits at the live market rate and a CPFP child lifts it if the market climbs) |
