...
GitHub may continue displaying default-branch alerts until this PR is merged and its dependency graph is rescanned. No alerts were dismissed to obtain these results. Dependabot stays report-only; version-update PRs remain disabled.
...
- Rust tests: 241 Pact workspace + 11 protocol + 8 Nostr + 6 Corkboard + 17 Crier + 21 desktop = **304 passed**; relay-prober compiles and its zero-test target passes.
...
- Full rebuilt-cache regtest sweep and hosted CI: pending. This draft is not ready until those results are recorded.

The local sweep was built with the SDK's OS-randomness feature already enabled; adding the explicit feature to the standalone mapping crate does not change the daemon's feature set. Browser tests and CI configuration were completed while that sweep ran.
