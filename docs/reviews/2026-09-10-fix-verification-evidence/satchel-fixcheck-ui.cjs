...
for(const locale of ['de-DE','en-US']) {
 ...
 for(const s of [{state:'completed',settled:false,source:'local'},{state:'refunded',settled:false,source:'local'},{state:'completed',settled:true,source:'local'}])console.log(JSON.stringify({swap:s,active:ex.isActive(s)}));
}
