...
| `test_v1_swap_ltc_leg_b` | v1 swap with litecoind as leg B: Litecoin Core (pre-25) rejects the `minconf` option hard-P2 relies on, so Bob's funding must fall back to the engine's own confirmed-input pre-selection and the swap completes. |
| `test_refund_survives_other_chain_outage` | Security review 2026-09-09 #9: with the timelocks passed and chain B's node stopped, Alice's tick still fires the T1 refund of leg A (the redeem-side chain-B queries are isolated from the refund; a chain-B error is reported only when no refund fired). |
...
| `test_taker_display_follows_rbf` | The display twin on the taker's side: Bob pins the maker's funding txid from `funded_a`, the maker RBFs it, the replacement confirms — asserted **before Bob's first tick**, his progress dock must not freeze at "their lock confirming 0/n" (`lock_confs` falls back to a script-scan when the pointer is dead); the swap then completes. |
| `test_settlement_rbf_race_redeem` | Post-mortem 2026-08-10: the redeem nurse RBF-bumps, then the **pre-bump version** is mined behind the engine's back (`generateblock` with the raw hex). The arm must adopt the mined winner (`settlement-pointer-heal`, `final_txid` repointed) and complete — not error-loop `-25` in `redeemed_b`. Bob still extracts the preimage from the winning tx. |
| `test_settlement_rbf_race_refund` | The refund twin: Bob's leg-B refund is bumped, the pre-bump version wins the race, the pointer heals and the refund converges; Alice reclaims leg A normally. |
| `test_sibling_funding_queue_v1` | Post-mortem 2026-08-09: two swaps off one single-coin wallet. One tick funds one swap and **queues** the other (`funding-queued` — confirmed coins short, own pending change covers it); the queue survives a shrunk C8 pre-funding timeout; the nurse bumps the parent freely (no swap descendants can exist); one confirmation frees the queue and both swaps complete. On the unfixed engine the second funding chained on the first's unconfirmed change — the orphaning hazard. |
| `test_sibling_funding_queue_v2` | v2 twin (security review 2026-09-09): two v2 swaps off one single-coin Core wallet. The first `adaptorfund` spends the lone confirmed coin; the second is refused as `funding queued` (never chained on the unconfirmed change — an RBF bump would orphan it together with the pre-signed MuSig2 redeems), the tick narrates `funding-queued` without aborting, and one confirmation lets the scheduler's retry arm fund leg A (`adaptor-fund`). |
| `test_funding_bump_descendant_belt` | The one chained shape hard-P2 still allows: an ordinary send the user chains on a funding's unconfirmed change. The nurse must refuse the RBF (`funding-bump-skipped-descendants`), the child survives in the mempool, and the swap completes. |
...
| `test_adaptor_direct_redeem_gate` | Security review 2026-09-09 #4: the direct `adaptorredeem` RPC enforces the scheduler's leg-B gate itself — refused (`REFUSING to reveal t`) while leg B sits at 0 confirmations, state stays `signed` and leg B unspent; accepted once it is `n_b` deep. |
